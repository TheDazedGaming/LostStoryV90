...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (selection == 0) {
			...
			cm.sendOk("#eYou can get Agent Point by doing various action in TropikMS such as getting a MSI or trading an item with Dargoth. Just play the Game normally and at some point you'll end up getting enough :)");
			cm.dispose();
			}
		} else if (status == 2) {
			if (selection == 0) {
