...
public class PlayerHandler {
    ...
    public static final void updateCharacter(final MapleCharacter chr) {
        //chr.updatePetAuto();
        //if (!chr.getCustomQuestData(170001).equals("") && chr.getPet(0) != null) {
        //    chr.getClient().getSession().write(PetPacket.loadExceptionList(chr.getId(), chr.getPet(0).getUniqueId(), chr.getCustomQuestData(170001)));
        //}
    }
        ...
        switch (actiontype) {
            case 0: { // Normal Key Map
                ...
                for (int i = 0; i < numChanges; i++) {
                    ...
                    if (type >= 0 && type <= 6) { // 0 = none, 1 = skill, 2 = item, 4 = UI, 5 = (attack, loot, sit, jump, npc chat), 6 = emotion
                        if (type == 1) { // I don't want to parse the keymap twice. :(
                            chr.changeKeybinding(key, type, action, (byte) (chr.isRBSkill(action) ? 1 : 0));
                        } else if (type != 0) {
...
//            chr.dropMessage(5, "This skill cannot be used with the current job as is not a reborn skill.");
...
//			chr.getMap().broadcastMessage(chr, MaplePacketCreator.showMagnet(mobId, slea.readByte()), chr.getPosition());
            ...
            default:
                ...
                if (effect.isMagicDoor()) { // Mystic Door
                    ...
                    if (chr.getBuffedValue(client.MapleBuffStat.MONSTER_RIDING) == null && chr.getDragon() != null) {
                        c.getSession().write(MaplePacketCreator.removeDragon(chr.getId()));
                        chr.getMap().broadcastMessage(MaplePacketCreator.removeDragon(chr.getId()));
                        chr.getMap().removeMapObject(chr.getDragon());
                        chr.setDragon(null);
                    }
...
//            return;
//        }
        if (attack.portals != chr.getPortalCount(false)) { // Portal count didn't match. Ignore 
            c.getSession().write(MaplePacketCreator.enableActions());
            return;
        }
        ...
        for (int i = 0; i < clones.length; i++) {
            if (clones[i].get() != null) {
                ...
                final AttackInfo attack2 = DamageParse.DivideAttack(attack, chr.isGM() ? 1 : OccupationConstants.getCloneDMG(chr.getOccupation()));

                CloneTimer.getInstance().schedule(new Runnable() {

                    public void run() {
                        if (attack2.portals != chr.getPortalCount(false)) { // Portal count didn't match. Ignore 
                            c.getSession().write(MaplePacketCreator.enableActions());
                            return;
                        }
...
//        }
        ...
        if (attack.AOE != 0 && chr.getBuffedValue(MapleBuffStat.SOULARROW) == null && attack.skill != 4111004 && !GameConstants.isVisitorSkill(attack.skill)) {
...
//	slea.readShort(); // Original Y pos
    ...
    public static final void BombSkill(SeekableLittleEndianAccessor slea, final MapleClient c) {
        ...
        final int charge = slea.readInt(); // 1200 for monster bomb and 800 for flashbang
        final int skillid = slea.readInt();
        ...
        if (skillid == 14111006) { // Poison bomb
            ...
            if (skill != null && c.getPlayer().getSkillLevel(skill) > 0) {
                Point newp = null;	// equation is something like -x^2/360+x for fully charged projectile, but i'm not doing rectline thing again.
                try {
                    newp = c.getPlayer().getMap().getGroundBelow(new Point(x + left * charge / 3, y - 30));
                } catch (NullPointerException e) {
                    newp = c.getPlayer().getPosition();
                }
                final MapleMist mist = new MapleMist(calculateBoundingBox(newp), c.getPlayer(), skill.getEffect(c.getPlayer().getSkillLevel(skill)));
