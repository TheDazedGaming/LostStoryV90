...
public class PlayersHandler {
                    ...
                    if (search_chr.getId() != chr.getId()) {
                        if (!search_chr.isGM() && !chr.isGM() || chr.isGM()) {
                            if (search_chr.getBuffedValue(MapleBuffStat.MORPH) == null) {
                                ...
                                search_chr.dropMessage(6, chr.getName() + " had played a prank on you!"); // there's a packet for this!
    ...
    public static void HitReactor(final SeekableLittleEndianAccessor slea, final MapleClient c) {
        ...
        if (reactor.getReactorId() == 5022000) {
            final String[] texts = {"What? The reactor didn't broke?", "Hmm..Maybe I should try and use another way instead.", "I wonder if the Antellion Relic will combine with it or not."};
    ...
    public static void RingAction(final SeekableLittleEndianAccessor slea, final MapleClient c) {
        ...
        if (mode == 0) {
            ...
            if (accepted) {
                ...
                MapleInventoryManipulator.addById(c, newItemId, (short) 1, "Obtained from engagement with " + chr.getName());
                MapleInventoryManipulator.removeById(chr.getClient(), MapleInventoryType.USE, chr.getMarriageItemId(), 1, false, false);
                MapleInventoryManipulator.addById(chr.getClient(), newItemId, (short) 1, "Obtained from engagement with " + c.getPlayer().getName());
