...
// todo dojo parties and subway
    public static void ExpeditionOperation(final SeekableLittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        switch (mode) {
            case 0x2D: // Creating
                ...
                if (chr.getParty() != null || et == null) {
                    c.getSession().write(MapleUserPackets.partyStatusMessage(PartyHandler.ALREADY_JOINED));
                    return;
                }
                ...
                c.getSession().write(MapleUserPackets.showExpedition(World.Party.getExped(party.getExpeditionId()), true, false));
                break;
            case 0x2E: // Invite
            ...
            case 0x2F: // Response
                ...
                if (action == 6 || action == 7) {
                    if (cfrom.getParty() == null || cfrom.getParty().getExpeditionId() <= 0) {
                        c.getSession().write(MapleUserPackets.partyStatusMessage(PartyHandler.NOT_IN_PARTY));
                        return;
                    }
                    if (action == 6) {
                    ...
                    if (action == 8) {
                        if (exped == null || chr.getParty() != null) {
                            if (chr.getParty() != null) {
                                cfrom.getClient().getSession().write(MapleUserPackets.expeditionStatusMessage(2, chr.getName()));
                            }
                            chr.dropMessage(1, "The expedition you are trying to join does not exist.");
                            return;
                        }
                        if (chr.getLevel() <= exped.getType().maxLevel && chr.getLevel() >= exped.getType().minLevel && exped.getAllMembers() < exped.getType().maxMembers) {
                            ...
                            if (partyId < 0) {
                                c.getSession().write(MapleUserPackets.partyStatusMessage(PartyHandler.PARTY_FULL));
                            } else if (partyId == 0) {
                                ...
                                World.Party.expedPacket(exped.getId(), MapleUserPackets.expeditionNotice(56, chr.getName()), new MaplePartyCharacter(chr));
                                World.Party.expedPacket(exped.getId(), MapleUserPackets.expeditionUpdate(exped.getIndex(partyN.getId()), partyN), null);
                            } else {
                    ...
                    for (Integer i : nowExped.getParties()) {
                        ...
                        if (expelled != null && expelled.isOnline()) {
                            ...
                            if (partyIndexTo < nowExped.getParties().size()) {
                                ...
                                player.getClient().getSession().write(MapleUserPackets.showExpedition(nowExped, false, true));
                                World.Party.expedPacket(nowExped.getId(), MapleUserPackets.expeditionUpdate(nowExped.getIndex(newParty.getId()), newParty), null);
                            }
                            if (chr.getEventInstance() != null && expelled.isOnline()) {
                                chr.getEventInstance().disbandParty();
                            }
