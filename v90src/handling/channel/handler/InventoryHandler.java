...
public class InventoryHandler {
    ...
    public static final boolean UseUpgradeScroll(final byte slot, final byte dst, final byte ws, final MapleClient c, final MapleCharacter chr, final byte type) {
        return UseUpgradeScroll(slot, dst, ws, c, chr, 0, type);
    }

    public static final boolean UseUpgradeScroll(final byte slot, final byte dst, final byte ws, final MapleClient c, final MapleCharacter chr, final int vegas, final byte type) {
        ...
        if (type == 0) {
            chr.getMap().broadcastMessage(chr, MaplePacketCreator.getNormalScrollEffect(chr.getId(), scrollSuccess, legendarySpirit, whiteScroll), vegas == 0);
        } else if (type == 1) {
            chr.getMap().broadcastMessage(chr, MaplePacketCreator.getPotentialScrollEffect(true, chr.getId(), scrollSuccess, legendarySpirit), vegas == 0);
        } else if (type == 2) {
            chr.getMap().broadcastMessage(chr, MaplePacketCreator.getPotentialScrollEffect(false, chr.getId(), scrollSuccess, legendarySpirit), vegas == 0);
		}
            ...
            switch (itemid) {
                case 2270004: { //Purification Marble
                    ...
                    if (mob.getHp() <= mob.getMobMaxHp() / 2) {
                        ...
                        MapleInventoryManipulator.addById(c, 4001169, (short) 1, "Obtained from Purification Marble on monster " + mob.getId());
                    } else {
                    ...
                    if (c.getPlayer().getInventory(MapleInventoryType.USE).getNumFreeSlot() >= 1) {
                        if (c.getPlayer().getInventory(MapleInventoryType.USE).countById(2430112) >= 25) {
                            if (MapleInventoryManipulator.checkSpace(c, 2049400, 1, "") && MapleInventoryManipulator.removeById(c, MapleInventoryType.USE, 2430112, 25, true, false)) {
                                MapleInventoryManipulator.addById(c, 2049400, (short) 1, "Scripted item: " + itemId + " on " + FileoutputUtil.CurrentReadable_Date());
                            } else {
                            ...
                            if (MapleInventoryManipulator.checkSpace(c, 2049400, 1, "") && MapleInventoryManipulator.removeById(c, MapleInventoryType.USE, 2430112, 10, true, false)) {
                                MapleInventoryManipulator.addById(c, 2049401, (short) 1, "Scripted item: " + itemId + " on " + FileoutputUtil.CurrentReadable_Date());
                            } else {
                                c.getPlayer().dropMessage(5, "Please make some space.");
                            }
                        } else {
    ...
    public static final void UseTreasureChest(final SeekableLittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (chr.getInventory(MapleInventoryType.CASH).countById(keyIDforRemoval) > 0) {
            ...
            if (GameConstants.gachaponRareItem(item.getItemId()) > 0) {
                World.Broadcast.broadcastMessage(MaplePacketCreator.getGachaponMega("[" + box + " Chest] " + c.getPlayer().getName(), " : Lucky winner of Gachapon!", item, (byte) 2));
            }
        } else {
            chr.dropMessage(5, "Please check your item inventory and see if you have a Master Key, or if the inventory is full.");
            c.getSession().write(MaplePacketCreator.enableActions());
    ...
    public static final void UseCashItem(final SeekableLittleEndianAccessor slea, final MapleClient c) {
        ...
        switch (itemId) {
            ...
            case 5062000: { //miracle cube
                ...
                if (item != null && c.getPlayer().getInventory(MapleInventoryType.USE).getNumFreeSlot() >= 1) {
                    ...
                    if (eq.getState() >= 5) {
                        ...
                        c.getPlayer().getMap().broadcastMessage(MaplePacketCreator.getMiracleCubeEffect(c.getPlayer().getId(), true));
                        c.getPlayer().forceReAddItem_NoUpdate(item, MapleInventoryType.EQUIP);
                        MapleInventoryManipulator.addById(c, 2430112, (short) 1, "Cubed on " + FileoutputUtil.CurrentReadable_Date());
                        used = true;
                    } else {
                        c.getPlayer().dropMessage(5, "This item's Potential cannot be reset.");
                    }
                } else {
                    c.getPlayer().getMap().broadcastMessage(MaplePacketCreator.getMiracleCubeEffect(c.getPlayer().getId(), false));
                }
                break;
            }
            ...
            case 5610000: { // Vega 30
                ...
                used = UseUpgradeScroll(src, dst, (byte) 2, c, c.getPlayer(), itemId, (byte) 0); //cannot use ws with vega but we dont care
                cc = used;
                break;
            }
    ...
    public static final void Pickup_Player(final SeekableLittleEndianAccessor slea, MapleClient c, final MapleCharacter chr) {
        ...
        boolean vac = false;
        double multiplier = OccupationConstants.getVortexRange(chr.getOccupation());
        ...
        try {
            ...
            if (vac && mapitem.isPlayerDrop()) { // Only can vac from mob drops (main item)
                vac = false;
            }
            if (!vac) {
                ...
                if (mapitem.getMeso() > 0) {
                    ...
                    if (MapleItemInformationProvider.getInstance().isPickupBlocked(mapitem.getItem().getItemId())) {
                        ...
                        if (mapitem.getItem().getQuantity() >= 50 && GameConstants.isUpgradeScroll(mapitem.getItem().getItemId())) {
                            FileoutputUtil.logUsers(chr.getName(), "Player picked up " + mapitem.getItem().getQuantity() + " of " + mapitem.getItem().getItemId());
                        }
                ...
                int i = 0;
                boolean isVortex = chr.getMapId() == 910000000 ? false : ((chr.getOccupation() / 100) == 4); // small vac only at fm
                List<MapleMapObject> items = chr.getMap().getItemsInRange(Client_Reportedpos, (GameConstants.maxViewRangeSq() * multiplier));
    ...
    private static void PickupItemAtSpot(final MapleClient c, final MapleCharacter chr, final MapleMapObject ob) {
        if (ob == null) {
            c.getSession().write(MaplePacketCreator.enableActions());
            return;
        }
        ...
        try {
            ...
            if (mapitem.isPlayerDrop() && mapitem.getOwner() != chr.getId()) { // cannot vacuum player drops
                c.getSession().write(MaplePacketCreator.enableActions());
                return;
            }
            if (mapitem.getMeso() > 0) {
                ...
                if (MapleItemInformationProvider.getInstance().isPickupBlocked(mapitem.getItem().getItemId())) {
                    ...
                    if (MapleInventoryManipulator.addFromDrop(c, mapitem.getItem(), true, mapitem.getDropper() instanceof MapleMonster)) {
						...
						if (mapitem.getDropper() instanceof MapleMonster) {
							if (c.getPlayer().getOccupation() / 100 == 4) {
								c.getPlayer().gainOccupationEXP(1); // need to pick up 32767 items to get max level
							}
						}
                    }
                } else {
    ...
    public static final void Pickup_Pet(final SeekableLittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        try {
            ...
            if (mapitem.getMeso() > 0) {
                if ((!chr.getStat().hasMeso && pet.getPetItemId() != 5000054) || petIgnore.contains(Integer.MAX_VALUE)) { // Ignore
                    c.getSession().write(MaplePacketCreator.enableActions());
                    return;
                }
                ...
                if ((!chr.getStat().hasItem && pet.getPetItemId() != 5000054) || petIgnore.contains(mapitem.getItem().getItemId())) {
                    c.getSession().write(MaplePacketCreator.enableActions());
                    return;
                }
                if (MapleItemInformationProvider.getInstance().isPickupBlocked(mapitem.getItem().getItemId())) {
                    ...
                    if (mapitem.getItem().getQuantity() >= 50 && GameConstants.isUpgradeScroll(mapitem.getItem().getItemId())) {
                        FileoutputUtil.logUsers(chr.getName(), "Pet picked up " + mapitem.getItem().getQuantity() + " of " + mapitem.getItem().getItemId());
                    }
                    if (MapleInventoryManipulator.addFromDrop(c, mapitem.getItem(), true, mapitem.getDropper() instanceof MapleMonster)) {
                        removeItem_Pet(chr, mapitem, petz);
                    }
    ...
    private static final boolean getIncubatedItems(MapleClient c) {
        ...
        MapleInventoryManipulator.addById(c, ids[z], (short) 1, ii.getName(ids[z]) + " on " + FileoutputUtil.CurrentReadable_Date());
        MapleInventoryManipulator.addById(c, ids[z_2], (short) 1, ii.getName(ids[z_2]) + " on " + FileoutputUtil.CurrentReadable_Date());
        return true;
    }
