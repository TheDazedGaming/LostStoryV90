    ...
    public static final void PlayerInteraction(final SeekableLittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        switch (action) { // Mode
            ...
            case ADD_ITEM: {
                ...
                final int price = slea.readShort();
...
//                    }
                    if (ivItem.getQuantity() >= bundles_perbundle) {
                        ...
                        if (bundles_perbundle >= 50 && GameConstants.isUpgradeScroll(ivItem.getItemId())) {
                            FileoutputUtil.logUsers(chr.getName(), "[PLAYER_SHOP_ADD_ITEM / ADD_ITEM] Placed " + bundles_perbundle + " of " + ivItem.getItemId());
                        }
                        if (GameConstants.isThrowingStar(ivItem.getItemId()) || GameConstants.isBullet(ivItem.getItemId())) {
                            ...
                            shop.addItem(new MaplePlayerShopItem(sellItem, (short) 1,(short)price));
                        } else {
                            MapleInventoryManipulator.removeFromSlot(c, type, slot, bundles_perbundle, true);

                            final IItem sellItem = ivItem.copy();
                            sellItem.setQuantity(perBundle);
                            shop.addItem(new MaplePlayerShopItem(sellItem, bundles,(short)price));
                        }
            ...
            case BUY_ITEM_HIREDMERCHANT: { // Buy and Merchant buy
                ...
                if (check > 32767 || check <= 0 || check2 > 32767 || check2 <= 0 || check3 > 32767 || check3 <= 0) { //This is the better way to check.
                    return;
                }
                if (quantity <= 0 || tobuy.bundles < quantity || (tobuy.bundles % quantity != 0 && GameConstants.isEquip(tobuy.item.getItemId())) // Buying
                        || chr.getItemQuantity(4000465, false) - (check2) < 0 || shop.getCoconut() + (check2) < 0) {
                    return;
                }
                if (quantity >= 50 && GameConstants.isUpgradeScroll(tobuy.item.getItemId())) {
                    FileoutputUtil.logUsers(chr.getName(), "[BUY_ITEM_PLAYER_SHOP / BUY_ITEM_STORE | BUY_ITEM_HIREDMERCHANT] Placed " + quantity + " of " + tobuy.item.getItemId());
                }
                ...
                if (item != null) {
                    if (item.bundles > 0) {
                        ...
                        if (item_get.getQuantity() >= 50 && GameConstants.isUpgradeScroll(item.item.getItemId())) {
                            FileoutputUtil.logUsers(chr.getName(), "[REMOVE_ITEM] Removed " + item_get.getQuantity() + " of " + item.item.getItemId());
                        }
            ...
            case MAINTANCE_ORGANISE: {
                ...
                if (imps != null && imps.isOwner(chr) && !(imps instanceof MapleMiniGame)) {
                    ...
                    if (chr.getMeso() + imps.getCoconut() < 0) {
                        c.getSession().write(PlayerShopPacket.shopItemUpdate(imps));
                    } else {
                        MapleInventoryManipulator.addById(c, 4000465,(short)imps.getCoconut(), "Gained from selling items with hired merchant");
                        imps.setCoconut((short)0);
                        c.getSession().write(PlayerShopPacket.shopItemUpdate(imps));
                    }
                }
                break;
