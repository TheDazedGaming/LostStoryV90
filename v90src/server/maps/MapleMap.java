...
//                return;
//            }
//            for (int numIncrements = 1; numIncrements < 1000000; numIncrements++) {
//                if (runningOid > 2100000000) {
//                    runningOid = 1000000000;
//                }
//                if (this.mapobjects.containsKey(Integer.valueOf(runningOid))) {
//                    runningOid++;
//                } else {
...
//    }
    ...
    private void dropFromMonster(final MapleCharacter chr, final MapleMonster mob) {
        ...
        final int cmServerrate = (int) (ChannelServer.getInstance(channel).getMesoRate() * OccupationConstants.getMesoMultiplier(chr.getOccupation()));
        final int chServerrate = (int) (ChannelServer.getInstance(channel).getDropRate() * OccupationConstants.getDropMultiplier(chr.getOccupation()));
        final int caServerrate = (int) (ChannelServer.getInstance(channel).getCashRate() * OccupationConstants.getCashMultiplier(chr.getOccupation()));

        final boolean isHunter = (chr.getOccupation() / 100 == 3);
        final Pair<Integer, Integer> chanceE = isHunter ? OccupationConstants.getHunterChance(chr.getOccupation()) : null;
        ...
        for (final MonsterDropEntry de : dropEntry) {
			...
			spawnMesoDrop(mesos * 2500, calcDropPos(pos, mob.getPosition()), mob, chr, false, droptype);
		    }
                    d++;
                } else {
                    int qqu = 0;
                    if (isHunter && chanceE != null && de.questid <= 0 && GameConstants.getInventoryType(de.itemId) == MapleInventoryType.ETC) {
                        if (chanceE.getLeft() == 100 || Randomizer.nextInt(100) < chanceE.getLeft()) {
                            qqu += chanceE.getRight();
                        }
                    }
