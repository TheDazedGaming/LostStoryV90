...
//	showdown = 100;
    ...
    private final void giveExpToCharacter(final MapleCharacter attacker, int exp, final boolean highestDamage, final int numExpSharers, final byte pty, final byte Class_Bonus_EXP_PERCENT, final byte Premium_Bonus_EXP_PERCENT, final int lastskillID, final boolean real) {
        ...
        if (exp > 0) {
            ...
            exp = (int) Math.min(Integer.MAX_VALUE, (exp * ChannelServer.getInstance(map.getChannel()).getExpRate()) * OccupationConstants.getExpMultiplier(attacker.getOccupation()));
        ...
        if (this.stats.isBoss() && (attacker.getOccupation() / 100 == 2)) { // gamer
            attacker.gainOccupationEXP(stats.dropsMeso() * 2);// lol 
        }
    }
