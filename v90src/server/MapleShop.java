...
public class MapleShop {
    ...
    public static MapleShop createFromDB(int id, boolean isShopId) {
        ...
        try {
            ...
            rs = ps.executeQuery();
            while (rs.next()) {
                ret.addItem(new MapleShopItem((byte) (rs.getByte("position") - 1), rs.getInt("itemid"), rs.getInt("price"), rs.getInt("reqitem"), rs.getInt("reqitemq"), rs.getShort("quantity"), rs.getInt("expiration"), rs.getShort("reqlevel")));
            }
            rs.close();
            ps.close();
