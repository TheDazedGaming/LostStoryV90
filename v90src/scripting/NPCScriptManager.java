...
public class NPCScriptManager extends AbstractScriptManager {
    ...
    public final void start(final MapleClient c, final int npc, final String file) {
        ...
        try {
            if (!cms.containsKey(c)) {
			    ...
			    Invocable iv = null;
				if (file != null) {
					iv = getInvocable("npc/" + file + ".js", c, true);
					fromFile = true;
				} else {
                ...
                if (iv == null) {
					if (file != null) {
						iv = getInvocable("npcNew/" + file + ".js", c, true);
						fromFile = true;
					} else {
						iv = getInvocable("npcNew/" + npc + ".js", c, true);
						fromFile = false;
					}
					if (iv == null) {
                ...
                final NPCConversationManager cm = new NPCConversationManager(c, npc, -1, (byte) -1, iv, fromFile ? file : null);
                cms.put(c, cm);
            ...
            FileoutputUtil.log("scripts\\batError.rtf", "Error executing NPC script, NPC ID : " + npc + "." + e);
            dispose(c);
        } finally {
            lock.unlock();
        }
    ...
    public final void action(final MapleClient c, final byte mode, final byte type, final int selection) {
        if (mode != -1) {
            ...
            try {
				...
				FileoutputUtil.log("scripts\\batError.rtf", "Error executing NPC script, NPC ID : " + cm.getNpc() + "." + e);
			} finally {
                lock.unlock();
            }
        }
    ...
    public final void dispose(final MapleClient c) {
        ...
        if (npccm != null) {
            ...
            if (npccm.getType() == -1) {
                ...
                c.removeScriptEngine("scripts/npcNew/" + npccm.getNpc() + ".js");
                c.removeScriptEngine("scripts/npc/notcoded.js");
				if (npccm.getFileName() != null) {
					c.removeScriptEngine("scripts/npc/" + npccm.getFileName() + ".js");
					c.removeScriptEngine("scripts/npcNew/" + npccm.getFileName() + ".js");
				}
            } else {
