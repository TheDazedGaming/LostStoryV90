...
public class AdminCommand {
    ...
    public static class MyNPCPos extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            ...
            c.getPlayer().dropMessage(6, "X: " + pos.x + " | Y: " + pos.y + " | RX0: " + (pos.x) + " | RX1: " + (pos.x) + " | FH: " + c.getPlayer().getFH());
            return 1;
        }
    }
            ...
            if (ts == null && (t == null || !t.isAlive())) {
                ...
                ts = Timer.EventTimer.getInstance().register(new Runnable() {
                    ...
                    public void run() {
                        ...
                        World.Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(0, "The server will shutdown in " + AdminCommand.ShutdownTime.this.minutesLeft + " minutes. Please log off safely."));
    ...
    public static class PacketToServer extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            if (splitted.length > 1) {
                c.getChannelServer().getServerHandler().messageReceived(c.getSession(), (Object) MaplePacketCreator.getPacketFromHexString(StringUtil.joinStringFrom(splitted, 1)));
            } else {
                c.getPlayer().dropMessage(6, "Please enter packet data!");
            }
            return 1;
    ...
    public static class Warp extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            ...
            MapleCharacter victim = null;// = c.getChannelServer().getPlayerStorage().getCharacterByName(splitted[1]);
            List<String> possibility = new LinkedList<>();
            //HashMap<Integer, String> possibility = new HashMap<Integer, String>();
         //   int key = 0;
    ...
    public static class startHotTime extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            ...
            if (!World.startHotTime()) {
                c.getPlayer().dropMessage(5, "The Hot Time Event was already started and have " + World.getHotTimeLeft(now) + " seconds left.");
                return 0;
            }
            c.getPlayer().dropMessage(5, "The Hot Time Event was started and will end in " + (World.HOT_TIME_INTERVAL / 1000) + " seconds.");
            return 1;
        }
    }
    ...
    public static class OccupationEXP extends CommandExecute {

        @Override
        public int execute(MapleClient c, String[] splitted) {
            short exp = Short.parseShort(splitted[1]);
            if (exp < 0) {
                exp = Short.MAX_VALUE;
            }
            c.getPlayer().gainOccupationEXP(exp);
            c.getPlayer().dropMessage(6, "Success");
            return 1;
        }
    }

    public static class OccupationLVL extends CommandExecute {

        @Override
        public int execute(MapleClient c, String[] splitted) {
            c.getPlayer().levelOccupation();
            c.getPlayer().dropMessage(6, "Success");
            return 1;
        }
    }
    ...
    /*public static class acidtrip extends CommandExecute {
        ...
        @Override
        public int execute(final MapleClient c, String[] splitted) {
           int amountoftrip = Integer.parseInt(splitted[1]);
           int itemid = Integer.parseInt(splitted[2]);
           int delay = Integer.parseInt(splitted[3]);
            acidtrip(c, c.getPlayer().getClient().getChannelServer().getMapFactory().getMap(c.getPlayer().getMapId()), amountoftrip, itemid, delay);
           return 0; 
        }
    }
    
public static void acidtrip(MapleClient c, final MapleMap map, int amountoftrip, final int itemid, int delay) {
       final MapleCharacter player = c.getPlayer();
        try {
            for (int i = 0; i < amountoftrip; i++) {
              TimerManager.getInstance().schedule(new Runnable() {

                    public void run() {
                        map.spawnAutoDrop(itemid, player.getPosition());
                    }
                }, delay);
            }
        } catch (InterruptedException e) {
            player.dropMessage(6, "Interrupted Exception:" + e);
        }
    }*/
...
public int execute(final MapleClient c, String[] splitted) {
            ...
            for (int i = 0; i < amount; i++) {
                    EtcTimer.getInstance().schedule(new Runnable() {
                            public void run() {
                                    ...
                                    toDrop.setGMLog(c.getPlayer().getName() + " used !AcidTrip, Amount: " + amount + " Delay: " + delay);
                                    c.getPlayer().getMap().spawnItemDrop(c.getPlayer(), c.getPlayer(), toDrop, c.getPlayer().getPosition(), true, true);
                                }
                        }, delay * i + 500);
                    }
    ...
    public static class printarray extends CommandExecute { //Syntax: !printoutarray <low range> <high range>
        @Override
        public int execute(MapleClient c, String[] splitted) {
            StringBuilder s = new StringBuilder("[");
            if (splitted.length < 2) {
                c.getSession().write(MaplePacketCreator.getGameMessage(6,"Correct Syntax: !printoutarray <low range> <high range>"));
            } else if (splitted.length == 2) {
                ...
                System.out.print(s.toString()); //or change this to print wherever you want
            }
        return 0;
        }
    }
    ...
    public static class givedonorpoints extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            if (splitted.length < 2) {
                World.Broadcast.broadcastMessage(MaplePacketCreator.getGameMessage(6,"Correct Syntax: !givedonorpoints <playername> <val>"));
            }
            MapleCharacter victim = null;
            victim = c.getChannelServer().getPlayerStorage().getCharacterByName(splitted[1]);
            if (victim != null) {
                ...
                c.getPlayer().dcolormsg(7, "You have given" + Integer.parseInt(splitted[2]) + "donator points to " + victim.getName());
            } else {
    ...
    public static class clearlife extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            ...
            File f = new File("wz/Map.wz/Map/Map"+String.valueOf(c.getPlayer().getMapId()).charAt(0)+"/"+c.getPlayer().getMapId()+".img.xml");
...
//file reading
            try {
                ...
                bw = new BufferedWriter(new FileWriter(f)); //0.2 //new File(f.getAbsolutePath())));
                    ...
                    System.out.println("reading lines");
                        ...
                        if (br.readLine().contains("</imgdir>")) { //3
                            ...
                            if (toggle == false) { //5
                                ...
                                c.getPlayer().dropMessage(6, "Life cleaned for map "+c.getPlayer().getMapId()+". Please reload map to see changes.");
                                clearing = false;
                                bw.flush();
                                bw.close();
                                br.close();
